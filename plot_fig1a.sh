...
if [ "$DIR" == "xiao_fetaltissues_2019" ]; then
    cp $DIR/macs2_results/${COND}.macs2_peaks.narrowPeak $m6a_bed
else
    set -- $DIR/macs2_results/${COND}_2.macs2_peaks.narrowPeak
    if [ -f "$DIR/macs2_results/${COND}_2.macs2_peaks.narrowPeak" ] || [ -f "$DIR/macs2_results/${COND}H_48h_2.macs2_peaks.narrowPeak" ] ; then
        echo '>=2 reps'
