...
if (peakcaller == 'metdiff'){
  peaks.overlap <- read.csv('fig1/peak_overlaps_summary_metdiff.txt',sep=' ')
}else {peaks.overlap <- read.csv('fig1/peak_overlaps_summary.txt',sep=' ')}
overlap.annotations <- read.csv('summary_files/fig1cd_exp_summary.txt',sep=' ')
...
for (cellline in unique(peaks.overlap$cellline1)){
  ...
  if (dim(cell.peak.ol)[1] > 0){
    ...
    pdf(paste0("fig1c_",cellline,"_peak_heatmap_metdiff.pdf"),width=3.5,height=3)
...
for (species in c("hg38")){
  ...
  for (exp1 in cell.peak.ol$explabel1){
    ...
    for (exp2 in cell.peak.ol[which(cell.peak.ol$explabel1 == exp1),'explabel2']){
      peak.overlap <- cell.peak.ol[which(cell.peak.ol$explabel1 == exp1 & cell.peak.ol$explabel2 == exp2),'peak_overlap'][1]
      total.peaks <- cell.peak.ol[which(cell.peak.ol$explabel1 == exp1 & cell.peak.ol$explabel2 == exp2),'total_peaks'][1]
      perc.overlap.mat[exp1,exp2] <- peak.overlap*100/total.peaks
    }
  }
}
...
pdf(paste0("fig1d_clustered_peak_heatmap_metdiff.pdf"),width=8,height=6)
