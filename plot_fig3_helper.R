get.gtf <- function(species,mm10.gtf,hg38.gtf){
  if (species == "hg38"){ return(hg38.gtf)
...
rowVar <- function(x) {
  rowSums((x - rowMeans(x))^2)/(dim(x)[2] - 1) #CHECK THIS
}
...
plot_overdispersion <- function(means,vars,label){
  png(paste0('fig3/sfig3a_',label,"_mean_variance.png"),width=100,height=100,units='mm',res=500)
