...
#plot SFig 1d DRAC enrichment
annotation.order <- c("utr3","utr5","exon","intron","utr3*","utr5*")
control.summary <- read.csv(exp.summary,sep=' ')
control.summary <- control.summary[which(control.summary$fig == 2 & control.summary$control_type == "negative"),]
...
for (expcont in control.summary$studycont){
  ...
  label <- control.summary[which(control.summary$studycont == expcont),'label']
  ...
  pdf(paste0('fig1/sfig1d_',exp,'_macs2_motif_enrichment.pdf'),height=3.2,width=4.3)
