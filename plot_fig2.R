...
outdir <- paste0('deq_output_',peakcaller)
experiment.summary <- read.csv(exp.summary,sep=' ')
experiment.summary <- experiment.summary[which(experiment.summary$fig == "2"),]
...
for (contexp in experiment.summary$studycont){ 
  ...
  treatment <- as.character(subexpsum$stimulus)
  if (peakcaller == "metdiff"){
    if (ctype == 'positive'){
      peak.files <- list.files(paste0(exp,'/',control,'_',treatment,'_MeTDiff_output/MeTDiff_output/'),pattern='peak.sorted.bed',full.names=TRUE)}else{ 
        peak.files <- list.files(paste0(exp,'/',control,'_MeTDiff_output/MeTDiff_output/'),pattern='peak.sorted.bed',full.names=TRUE) } 
    }else {
      peak.files <- c(list.files(paste0(exp,'/macs2_results/'),pattern=paste0(control,'_*.bed'),full.names=TRUE),
                      list.files(paste0(exp,'/macs2_results/'),pattern=paste0(stimulus,'_*.bed'),full.names=TRUE))

    }
  
  control <- subexpsum$control
  treatment <- subexpsum$stimulus 
  input.bams <-list.files(paste0(exp,'/alignments/'),pattern=paste0(control,'_input_*.star.sorted.bam$'),full.names=TRUE)
  ip.bams <- list.files(paste0(exp,'/alignments/'),pattern=paste0(control,'_IP_*.star.sorted.bam$'),full.names=TRUE)
  treatment.input.bams <-list.files(paste0(exp,'/alignments/'),pattern=paste0(stimulus,'_input_*.star.sorted.bam$'),full.names=TRUE)
  treatment.ip.bams <- list.files(paste0(exp,'/alignments/'),pattern=paste0(stimulus,'_IP_*.star.sorted.bam$'),full.names=TRUE)
...
for (exp in control.summary[which(control.summary$control_type == 'negative'),'study']){
  ...
  col.data <- data.frame("IP" = rep("IP",length(),rep("input",),"Factor"=,"Condition"=) #FIX THIS
...
pdf(paste0('sfig2_peak_diff_distributions_',peakcaller,'.pdf'),height=4,width=6)
...
pdf(paste0('sfig2_peak_diff_distributions_overlap_',peakcaller,'.pdf'),height=4,width=6)
